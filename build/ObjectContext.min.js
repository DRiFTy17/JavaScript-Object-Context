...
ObjectContext.prototype._doesObjectHaveChanges=function(a){return 0<a.changeset.length||a.current._objectMeta.status===ObjectContext.ObjectStatus.New||a.current._objectMeta.status===ObjectContext.ObjectStatus.Modified||a.current._objectMeta.status===ObjectContext.ObjectStatus.Deleted};ObjectContext.prototype.doesObjectExist=function(a){if(!a)return!1;for(var b=0;b<this._objectMap.length;b++)if(this._objectMap[b].current===a)return!0;return!1};
...
1)):this._objectMap[c].current._objectMeta.status!==ObjectContext.ObjectStatus.New&&(this._objectMap[c].current._objectMeta.status=ObjectContext.ObjectStatus.Deleted);for(c=this._objectMap.length-1;0<=c;c--){var d=this._objectMap[c];d.current!==a&&d.rootParent===a&&(!0===b?this._objectMap.splice(c,1):d.current._objectMeta.status!==ObjectContext.ObjectStatus.New&&(d.current._objectMeta.status=ObjectContext.ObjectStatus.Deleted))}this.evaluate()};
ObjectContext.prototype.removeAll=function(a){if(!0===a)this._objectMap=[];else for(a=0;a<this._objectMap.length;a++)this._objectMap[a].current._objectMeta.status=ObjectContext.ObjectStatus.Deleted;this.evaluate()};ObjectContext.prototype._getMapIndex=function(a){for(var b=0;b<this._objectMap.length;b++)if(this._objectMap[b].current===a)return b;return null};
ObjectContext.prototype.getOriginal=function(a){for(var b=0;b<this._objectMap.length;b++)if(this._objectMap[b].current===a)return this._deepCopy(this._objectMap[b].original);return null};ObjectContext.prototype._getMappedObject=function(a){a=this._getMapIndex(a);if(null===a)throw Error(this.stringFormat("Invalid object index: {0}",a));return this._objectMap[a]};
...
ObjectContext.prototype.revert=function(a){if(!a)throw Error("Invalid object provided.");a=this._getMapIndex(a);if(null===a)throw Error("Could not determine object index. Revert changes failed.");this._resetObject(this._objectMap[a]);this.evaluate()};ObjectContext.prototype.revertAll=function(){for(var a=0;a<this._objectMap.length;a++)this._resetObject(this._objectMap[a]);this.evaluate()};
